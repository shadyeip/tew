...
This flag is useful when you have a comprehensive Nmap scan that identifies more IPs than those resolved by DNS lookups. The `--include-orphaned-ips` flag ensures that only IP addresses without an associated domain are included in the output. This must be ran with the `--vhost` flag.
